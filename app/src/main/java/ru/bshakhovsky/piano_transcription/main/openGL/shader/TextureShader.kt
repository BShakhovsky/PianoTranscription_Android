...
class TextureShader(assets: AssetManager) : Shader(assets, "2D") {
    ...
    companion object {
        fun sendTexture(
            ...
            GLES.glActiveTexture(GLES.GL_TEXTURE0 + buffNo)
            textures.bindTexture(buffNo)
            GLES.glUniform1i(frameBuff, buffNo)
            GLES.glVertexAttribPointer(
                texPos, 2, GLES.GL_FLOAT, false, 0, Utils.allocFloat(
    ...
    private val texPos = attribute("texturePos").also { GLES.glEnableVertexAttribArray(it) }
    ...
    fun draw(textures: Texture, buffNo: Int, toReflect: Boolean = false) {
        ...
        GLES.glVertexAttribPointer(
            pos, 2, GLES.GL_FLOAT, false, 0, Utils.allocFloat(@Suppress("Reformat")
                floatArrayOf(-1f, -1f,   1f, -1f,   -1f, 1f,        -1f, 1f,   1f, -1f,   1f, 1f)
            )
        )
        GLES.glDrawArrays(GLES.GL_TRIANGLES, 0, 6)
    }
}