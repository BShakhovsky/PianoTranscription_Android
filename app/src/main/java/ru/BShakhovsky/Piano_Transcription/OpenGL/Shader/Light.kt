...
class Light(val lightDir: FloatArray, index: Int, mainShader: Shader, shadowReversed: Boolean = false) {
    ...
    val lightMVO   = mainShader.uniform("lightMVO$index")
    ...
    init { FloatArray(16).also { view -> Matrix.setLookAtM(view, 0,
        ...
        FloatArray(4).also { viewCords -> for (cords in arrayOf(
            ...
            for (i in 0..viewCords.lastIndex) viewCords[i] /= viewCords.last()
              xLeft =   xLeft.coerceAtMost(viewCords[0]); xRight = xRight.coerceAtLeast(viewCords[0])
            yBottom = yBottom.coerceAtMost(viewCords[1]);   yTop =   yTop.coerceAtLeast(viewCords[1])
               zMin =    zMin.coerceAtMost(viewCords[2]);   zMax =   zMax.coerceAtLeast(viewCords[2])
        } }
        FloatArray(16).also { ortho -> Matrix.orthoM(ortho, 0, xLeft, xRight, yBottom, yTop, -zMax, -zMin)
            Matrix.multiplyMM(lightOrtho, 0, ortho, 0, view, 0) }

        orthoWidth = (xRight - xLeft).toInt(); orthoHeight = (yTop - yBottom).toInt()
    } }
}