...
class Texture(context: Context, lights: Array<Light>) {
    ...
    private val buff = IntArray(4); private val texture = IntArray(buff.size + 1)
    private val depthBuff = IntArray(buff.size)

    init {
        GLES32.glGenFramebuffers(buff.size, buff, 0); GLES32.glGenTextures(texture.size, texture, 0)
        GLES32.glGenRenderbuffers(depthBuff.size, depthBuff, 0)
        for (i in 0..buff.lastIndex) { bindBuff(i)
            size(if (i >= lights.size) 0 else lights[i].orthoWidth,
                 if (i >= lights.size) 0 else lights[i].orthoHeight, i)
            GLES32.glFramebufferRenderbuffer(GLES32.GL_FRAMEBUFFER, GLES32.GL_DEPTH_ATTACHMENT, GLES32.GL_RENDERBUFFER, depthBuff[i])
    ...
    private fun size(newWidth: Int, newHeight: Int, i: Int = buff.lastIndex) {
        ...
        GLES32.glTexImage2D(GLES32.GL_TEXTURE_2D, 0, GLES32.GL_RGBA, newWidth, newHeight, 0, GLES32.GL_RGBA, GLES32.GL_UNSIGNED_BYTE, null)
    }

    private fun parameteri() {
        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_MIN_FILTER, GLES32.GL_LINEAR) // or NEAREST
//        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_MAG_FILTER, GLES32.GL_LINEAR) // or NEAREST
//        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_WRAP_S, GLES32.GL_CLAMP_TO_BORDER) // or EDGE
//        GLES32.glTexParameteri(GLES32.GL_TEXTURE_2D, GLES32.GL_TEXTURE_WRAP_T, GLES32.GL_CLAMP_TO_BORDER) // or EDGE
    }
}