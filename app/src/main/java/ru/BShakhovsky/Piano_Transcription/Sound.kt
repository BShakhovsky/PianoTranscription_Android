...
class Sound(private val context: Context, private val bar: ProgressBar, private val count: TextView) : SoundPool.OnLoadCompleteListener {
    ...
    init {
        ...
        notes = intArrayOf(sound.load(context, raw.note00, 1),
    ...
    override fun onLoadComplete(sound: SoundPool?, note: Int, status: Int) { with(loaded) {
        Assert.state(!contains(note) and (size in 0..87) and (status == 0))
        add(note)
        bar.progress = size; bar.secondaryProgress = (size + 88) / 2
    ...
    fun play(note: Int, velocity: Float = 1f) {
        ...
        sound.play(notes[note], velocity * (notes.lastIndex - note) / notes.lastIndex.toFloat(),
            velocity * note / notes.lastIndex.toFloat(), 1, 0, 1f)
    }
    fun stop(note: Int) = sound.stop(notes[note])
}